...
export default {
  ...
  methods: {
    getAngebote(query, sortBy, price, type) {
      getAngebote(query, sortBy, price, type).then((response) => {
        this.offers = response;
      });
    },
    getServices(query, type, sortBy) {
      getServices(query, type, sortBy).then((response) => {
  ...
  created() {
    ...
    getAngebote(query, sortBy, price, type).then((responseOffers) => {
      this.offers = responseOffers;

      getServices(query, type, sortBy).then((responseServices) => {
  ...
  watch: {
    $route(to, from) {
      if (
        ...
        to.query.sortBy != from.query.sortBy ||
        to.query.price != from.query.price ||
        ...
        this.getServices(to.query.query, to.query.type, to.query.sortBy);
      }
    },
  },
};
